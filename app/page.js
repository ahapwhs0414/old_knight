...
그는 왕국에서 가장 오래된 기사였다.
어린 시절부터 검을 들었고, 늙어서는 왕과 여왕을 지켰다. 왕자가 태어났을 때는 밤새 곁을 지켰고, 공주가 처음 걸음마를 뗐을 때는 혹시 넘어질까 뒤를 따라다녔다.
...
그는 낡은 갑옷 대신 거친 털가죽 같은 외투를 몸에 두르고 있었다. 얼굴에는 오래된 상처 자국이 여러 개 남아 있었고, 귀 가장자리는 군데군데 찢어져 있었다. 야영지 주변에는 비슷한 차림의 용병들이 흩어져 누워 있었다. 어떤 이는 몸을 둥글게 말고 잠들어 있었고, 어떤 이는 땅에 코를 가까이 댄 채 주변 냄새를 살피고 있었다.
...
기사는 그제야 괴물의 모습을 보았다.
그것은 거대한 쇳덩이였다.
네 개의 둥근 발을 쉼 없이 굴리며 길 위를 미끄러지듯 달리고 있었다. 앞쪽에는 투명한 방패가 달려 있었고, 그 안에서는 귀족들이 편안한 의자에 앉아 있었다.
괴물은 뜨거운 숨을 내뿜으며 굉음을 토해냈다. 쇠 냄새와 타는 냄새가 뒤섞인 기묘한 향이 길 위에 남았다.
...
그때 무장한 용병들이 나타났다.
그들은 모두 제각기 다른 갑옷을 걸치고 있었지만 어딘가 비슷한 분위기를 풍겼다. 몸에는 오래된 상처가 많았고, 털은 거칠고 엉켜 있었다. 그들 중 몇몇은 낮게 으르렁거리듯 목을 울렸고, 또 다른 이들은 땅을 긁으며 경계심 가득한 눈빛을 보냈다.
